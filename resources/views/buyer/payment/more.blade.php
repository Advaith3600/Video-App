    ...
    <div id="advanced_table_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                ...
                <div class="filter-item">
                    <form action="{{ guard_url('invoice/invoice') }}" method="GET" enctype="multipart/form-data">
                        ...
                        <button type="{{$search==null?'button':'submit'}}" class="btn btn-warning" 
                            ...
                            title="{{$search==null?'Apply search criteria such as payment status and date range prior to downloading excel':''}}"><i class="las la-download"></i></a>Download
                            Excel Report</button>
                        </form>
                </div>
            </div>
                    ...
                    <thead>
                        <tr role="row">
                            ...
                            <th tabindex="0" aria-controls="advanced_table" rowspan="1" colspan="1"
                                aria-label="#: activate to sort column ascending" style="width: 201px;">Invoice Number</th>
                            <th tabindex="0" aria-controls="advanced_table" rowspan="1" colspan="1"
                                aria-label="Amount: activate to sort column ascending" style="width: 157px;">Supplier</th>
                            <th tabindex="0" aria-controls="advanced_table" rowspan="1" colspan="1"
                                aria-label="Order Date: activate to sort column ascending" style="width: 195px;">Issue Date
                            </th>
                            <th tabindex="0" aria-controls="advanced_table" rowspan="1" colspan="1"
                                aria-label="Amount: activate to sort column ascending" style="width: 157px;">Due Date</th>
                            
                            <th tabindex="0" aria-controls="advanced_table" rowspan="1" colspan="1"
                                aria-label="Payment Status: activate to sort column ascending" style="width: 262px;">Payment
                                Status</th>
                            <th tabindex="0" aria-controls="advanced_table" rowspan="1" colspan="1"
                                aria-label="Amount: activate to sort column ascending" style="width: 157px;">Amount</th>
                            <th tabindex="0" aria-controls="advanced_table" rowspan="1" colspan="1"
                                aria-label="Amount: activate to sort column ascending" style="width: 157px;">PO Number</th>
                            <th class="text-right" tabindex="0" aria-controls="advanced_table" rowspan="1"
                                colspan="1" aria-label="Actions: activate to sort column ascending" style="width: 175px;">
                                Actions</th>
                            <th tabindex="0" aria-controls="advanced_table" rowspan="1" colspan=""
                                aria-label="Amount: activate to sort column ascending" style="width: 157px;"></th>
                    ...
                    <tbody>
                        @foreach($data as $key=>$value)
                            <tr role="row" class="odd">
                                ...
                                <td>{{@$value->seller->company_name}}</td>
                                <td>{{date('d M y',strtotime($value->created_at))}}</td>
                                <td>{{date('d M y',strtotime($value->created_at->addDays(@$value->seller->payment_terms_delay)))}}</td>
                                <td><span class="label label-{{@$value->payment_status=='Paid'?'success':''}}{{@$value->payment_status=='Pending'?'warning':''}}{{@$value->payment_status=='Not Paid'?'danger':''}}{{@$value->payment_status=='Failed'?'danger':''}}{{@$value->payment_status=='Bank Processing'?'primary':''}}" {{@$value->payment_status=='Batched'?'style=background-color:#e4e444;':''}}{{@$value->payment_status=='Not Paid'?'style=background-color:#32efdd;':''}}>{{@$value->payment_status}}</span></td>
                                <td>${{@number_format(($value->total + $value->transaction_charge),2)}}</td>
                                <td>{{@$value->coupa_po_number}}</td>
                                
                                <td class="text-right" colspan="2">
                                    <div class="table-actions">
                                            @if(user()->isBuyer()&&$value->payment_status=='Not Paid')
                                            <a class="btn-primary" id="{{$value->id}}" style="font-size:14px;" href="{{trans_url('buyer/coupa_invoice/'.$value->getRouteKey())}}" target="_blank">Pay</a>
                                            @endif
                                            @if(user()->isBuyer()&&$value->payment_status=='Not Paid'&&$value->order_status=='Confirmed')
                                            <button class="btn btn-primary pay_now" id="{{$value->id}}">Pay</button>
                                            @endif
                                            <a href="{{guard_url('order/order?id='.hashids_encode($value->id))}}" data-toggle="tooltip" data-placement="left" title=""
                                            data-original-title="View Order"><i
                                                class="las la-box"></i></a>
                                            <a class="invoiceview" id="{{$value->getRoutekey()}}" data-toggle="modal" data-backdrop="static" data-target="#invoiceModal"  data-placement="left" title="View Invoice"
                                            data-original-title="View Invoice" target="_blank"><i
                                                class="las la-eye"></i></a>
                                            <a href="{{guard_url('invoice/download_invoice/'.$value->id)}}" data-toggle="tooltip" data-placement="left" title=""
                                            data-original-title="Download Invoice"><i
        ...
        <div class="row">
            <div class="col-sm-12 col-md-5">
                <div class="dataTables_info" id="advanced_table_info" role="status" aria-live="polite" style="align:right;">Showing {{($data->links()->paginator->currentPage()-1)*$data->links()->paginator->perPage()+1}} to {{($data->links()->paginator->currentPage()-1)*$data->links()->paginator->perPage()+count($data->links()->paginator->items())}} of {{$data->links()->paginator->total()}}
                    entries</div>
            </div>
...
<script>
    ...
    $('#payment_status,#invoice_number,#coupa_po_number,#seller_id').on('change',function(){
        ...
        data : {payment_status:payment_status,seller_id:seller_id,start_date:start_date,end_date:end_date,coupa_po_number:coupa_po_number,invoice_number:invoice_number},
